...
if avail git;then
	...
	__git_ps1(){
		...
		local s
		local cnt
		s="$(git -c color.ui=always status -s -uno|__head_n_count)"
		cnt=$?
		# echo "debug: $s, $cnt" >&2
		if test "$cnt" -eq 0; then
			s="$__GIT_STATUS_CLEAN"
...
if test "$MSYSTEM"; then
	...
	PS1="$TESC$USER@$(</proc/sys/kernel/hostname):\$(__pwd)$TEND"
fi
# special prefix
if test "$debian_chroot"; then
	PS1="$PS1$C2($debian_chroot)$C_END"
fi
# the usual user@host:pwd, no u/h for msys and termux though
if test -z "$MSYSTEM" -a -z "$TERMUX_VERSION"; then
	PS1="$PS1$C0$USER$C1@$C0$(</proc/sys/kernel/hostname)$C1:\$(__pwd)$C_END"
else
...
unset PROMPT_CHAR C0 C1 C2 C_END TESC TEND MTITLE
unset -f __color_seq
...
if test "$MSYSTEM"; then
	...
	# add scoop neovide/nvim to PATH
	# to do: detect scoop location
	SCOOP=/d/scoop/apps
	for d in $SCOOP/neovim/current/bin $SCOOP/neovide/current; do
		if test -d "$d"; then
			export PATH=$PATH:"$d"
		fi
	done
...
if avail apt;then
	...
	alias pu='apt upgrade --no-install-recommends'
...
alias cfct='curl $CURL_OPTS https://cloudflare.com/cdn-cgi/trace'
